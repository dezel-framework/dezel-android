...
static void
displayNodeResolveMarginsCallback(DisplayNodeRef node)
{
	const auto wrapper = reinterpret_cast<DisplayNodeWrapperRef>(DisplayNodeGetData(node));
	...
	JNI_CALL_VOID_METHOD(
		wrapper->env,
		wrapper->object,
		DisplayNodeResolveMarginsMethod
	);
}

static void
displayNodeResolveBordersCallback(DisplayNodeRef node)
{
	const auto wrapper = reinterpret_cast<DisplayNodeWrapperRef>(DisplayNodeGetData(node));
	...
	JNI_CALL_VOID_METHOD(
		wrapper->env,
		wrapper->object,
		DisplayNodeResolveBordersMethod
	);
}
...
DisplayNodeWrapperCreate(JNIEnv* env, jobject object, DisplayNodeRef node)
{
	...
	DisplayNodeSetResolveMarginsCallback(node, &displayNodeResolveMarginsCallback);
	DisplayNodeSetResolveBordersCallback(node, &displayNodeResolveBordersCallback);
