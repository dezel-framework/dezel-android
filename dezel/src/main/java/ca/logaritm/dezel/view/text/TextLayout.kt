...
import ca.logaritm.dezel.view.text.layout.StaticLayoutBuilder
...
open class TextLayout {
	...
	/**
	 * Retrieves the string at a specified point.
	 * @method string
	 * @since 0.1.0
	 */
	open fun string(x: Float, y: Float): Spanned? {
	...
	/**
	 * Builds the layout in the specified bounds.
	 * @method build
	 * @since 0.1.0
	 */
	...
	/**
	 * Draws the layout in the specified canvas.
	 * @method draw
	 * @since 0.1.0
	 */
	open fun draw(canvas: Canvas) {
		...
		when (this.textAlign) {
			...
			TextAlign.MIDDLE_CENTER -> offset += extent.toMiddleOf(bounds).y

			TextAlign.BOTTOM_LEFT,
			TextAlign.BOTTOM_RIGHT,
			TextAlign.BOTTOM_CENTER -> offset += extent.toBottomOf(bounds).y
